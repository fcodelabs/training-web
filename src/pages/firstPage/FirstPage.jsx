import React, {useState} from 'react'
import Button from '@mui/material/Button'
import TextField from '@mui/material/TextField';
import './FirstPage.css'
import { useNavigate } from 'react-router-dom';

const FirstPage = () => {
    const navigate = useNavigate();

    const nameList = ['Liam', 'Logan', 'Benjamin', 'James', 'Noah', 'William']
    const [selectedName, setSelectedName] = useState('')

    const selectRandomName = () => {
        var randName = nameList[Math.floor(Math.random() * nameList.length)];
        setSelectedName(randName)
    }

    const handleName = () => {
        if(selectedName.trim() !== ''){
            navigate('/diaryHome', {state: selectedName})
        }else{
            console.log("Please enter name");
        }
    }

    return (
        <div>
            <div className="signUpCardContainer size">
                <div className='signUpCard'>
                    <div className='signUpCardContent'>
                        <div className='logoTitle field'>
                            <div className='logo'>
                                <img src='data:image/svg+xml;base64,'
                                alt='dear diary'/>
                            </div>
                            <div className='diaryName'>Dear Diary</div>
                        </div>
                        <div className='signUpCardTitle field'>Sign In</div>
                        <div className='signUpCardInputContent field'>
                            <div className='inputFieldContainer'>
                                <div className='signUpCardInputField'>
                                    <TextField
                                        required
                                        id="outlined-required"
                                        label="Your Nickname"
                                        placeholder='Your Nickname*'
                                        value={selectedName}
                                        onChange={ (e) => setSelectedName(e.target.value)}
                                    />
                                </div>
                                <div className='randomButton'>
                                    <Button 
                                        variant='contained' 
                                        disableElevation
                                        onClick={selectRandomName}
                                    >Random</Button>
                                </div>
                            </div>
                        </div>
                        <div className='signInButton field'>
                            <div className='btnbtnClass'>
                                <Button 
                                    variant='contained' 
                                    disableElevation
                                    disabled ={!selectedName}
                                    onClick={handleName}
                                >Continue</Button>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default FirstPage
